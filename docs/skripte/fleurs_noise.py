"""Lohnt es sich, vor dem Encoder zu entrauschen?

Die naheliegende Sorge: Entrauscher sind auf das menschliche Ohr optimiert und
hinterlassen Artefakte, die ein ASR-Modell nie im Training gesehen hat. Die
naheliegende Hoffnung: eine verrauschte Aufnahme wird lesbarer. Beides ist
messbar, wenn man das Rauschen selbst hinzufügt und die Wahrheit kennt.

FLEURS ist sauber, also wird kontrolliert verrauscht -- wahlweise mit weissem
Rauschen oder mit einem echten Störteppich aus einer der Testaufnahmen -- und
danach jede Entrausch-Kette gegen "gar nichts tun" gehalten.

    python docs/skripte/fleurs_noise.py <n> <build> <snr_db> <cond> [cond ...]
    python docs/skripte/fleurs_noise.py 150 models/voxtral-mini-8bit 10 raw afftdn anlmdn
"""
import json
import pathlib
import sys
...
def main():
    ...
    print(f"# FLEURS de_de test, {len(clips)} Aufnahmen, {sec/60:.1f} min, "
          f"weisses Rauschen bei {snr:.0f} dB SNR")
    print(f"# Build: {build}\n")
    print(f"{'Bedingung':14s} {'WER':>7s} {'CER':>7s} {'Speed':>7s}")
    ...
    print("\nGegen 'raw' (verrauscht, unbehandelt), gepaart ueber Aufnahmen:")
    base = store["raw"]
