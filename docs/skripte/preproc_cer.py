...
docs/skripte/wer.py answers "which build hears better" on a fixed audio file.
...
land in /tmp/wer_<variant>.txt so docs/skripte/bootstrap_cer.py can put a
confidence interval on any pair without changes.

    python docs/skripte/preproc_cer.py <reference.txt> <source-audio> <t0> <t1> \
        <build> <variant> [variant ...]

    python docs/skripte/preproc_cer.py Audiotest2/referenz/hart_780-900_REFERENZ.txt \
        "Audiotest2/Romy Podcast 323.m4a" 780 900 models/voxtral-mini-8bit \
...
def main():
    ...
    print(f"# {ref_path}: {len(ref)} Woerter / {len(ref_chars)} Zeichen, "
          f"{t1 - t0:.0f}s aus {pathlib.Path(src).name}")
    print(f"# Build: {build_path}\n")
    print(f"{'Variante':16s} {'peak':>6s} {'LUFS':>7s} {'WER':>7s} {'CER':>7s} "
