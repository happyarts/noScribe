...
differing spans are at character level:
  ...
  orthographic  short span, characters nearly the same   ("Balance Öl"/"Balanceöl")
...
is not buying recognition quality.
    ...
    python docs/skripte/encoder_diff.py <reference-build> <build> [build ...] \
        --audio <file> [file ...]
...
for b in builds[1:]:
    ...
    print(f"{words_r} / {words_o} Woerter, {len(events)} abweichende Stellen\n")
    for kind in ("orthographic", "ambiguous", "lexical", "omission", "cascade"):
        n = counts.get(kind, 0)
        share = (f"{n / decisive * 100:5.1f}% der klassifizierbaren"
                 if decisive and kind != "cascade" else "")
        print(f"  {kind:14s} {n:5d}   {n / max(1, words_r) * 1000:6.2f} je 1000 Woerter   {share}")
    if decisive:
        print(f"\n  -> akustisch (lexical+omission): {acoustic}/{decisive} = "
              f"{acoustic / decisive * 100:.1f}% der klassifizierbaren Stellen, "
              f"{acoustic / max(1, words_r) * 1000:.2f} je 1000 Woerter")
        print("     Ist dieser Anteil klein, aendert die Encoder-Praezision nur die "
              "Schreibweise, nicht das Gehoerte.")
