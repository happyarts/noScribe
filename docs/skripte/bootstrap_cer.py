...
Input is the transcripts that docs/skripte/wer.py leaves in /tmp/wer_<build>.txt.

    python docs/skripte/wer.py <reference.txt> <audio.wav> <build> [build ...]
    python docs/skripte/bootstrap_cer.py <reference.txt> <build> [build ...]
...
print(f"# Referenz {len(ref_words)} Woerter / {len(ref_chars)} Zeichen")
print(f"# {RESAMPLES} Resamples, Bloecke: {WORD_BLOCK} Woerter / {CHAR_BLOCK} Zeichen, Seed {SEED}\n")
...
for name in names:
    ...
    try:
        ...
        print(f"!! {path} fehlt -- erst docs/skripte/wer.py laufen lassen")
        continue
