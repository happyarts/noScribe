...
Stand: 24. August 2026 · M1 Max · PyAV 12.3.0, mlx-voxtral 0.0.6, Build
`voxtral-mini-8bit`. Abschnitte 1–5 gemessen am 28. Juli 2026. Alle Zahlen
gemessen, keine geschätzt.

**Ergebnis in einem Satz: am Audiopfad — von der Datei bis zum float32-Array —
ist nichts zu verbessern.** Jeder dort geprüfte Eingriff ist entweder
wirkungslos oder schädlich. Der einzige geprüfte Eingriff, der etwas bringt,
sitzt eine Stufe später im Merkmalspfad: der Clamp-Boden des log-Mel
(Abschnitt 6, eingebaut). Was die Erkennung sonst bewegt, liegt vor unserer
Pipeline — in der Qualität, in der die Aufnahme bei uns ankommt.
...
Zurückgelesen wird in `noScribe/voxtral_engine.py` als float32 — `sf.read(...,
dtype="float32")`, soundfile skaliert die int16-Werte selbst.
...
Dieselbe temporäre WAV füttert auch pyannote (`_run_diarize_subprocess` in
`main.py`) und die VAD-Pausenkorrektur (`decode_audio` ebendort) — eine Änderung am Zwischenformat wäre
nicht Voxtral-lokal.
...
| **Clamp-Boden aus einem Perzentil statt dem Maximum** | **eingebaut** — verhindert +1,52 [+0,78, +2,26] bei einem Türknall, kostet auf sauberem Material −0,12 [−0,85, +0,49] | Abschnitt 6 |
...
Das log-Mel wird nach unten geklemmt, und die int16-Zwischenstufe ist nur dann
sichtbar, wenn ihr Rauschboden über diesem Clamp liegt. Im Mel-Raum kostet die
Quantisierung 1,2 · 10⁻⁴ bei 0 dBFS, 2,0 · 10⁻³ bei −24 dBFS und 1,9 · 10⁻² bei
−36 dBFS — der Fehler wächst also, je leiser ausgesteuert wird.

**Der Abstand ist seit dem Perzentil-Boden kleiner, als diese Rechnung einmal
annahm.** Sie ging vom Maximum-Boden aus: 80 dB unter der lautesten Mel-Zelle,
gegen einen 16-Bit-Rauschboden bei −96 dBFS, also 16 dB Reserve, woraus sich
eine Schwelle bei etwa −24 dBFS Spitzenpegel ergab. Der Produktionspfad klemmt
inzwischen bei `Perzentil99 − 8`, und dieses Perzentil liegt auf echtem Material
17,6 dB (`speech_60s.wav`) bis 18,0 dB (`bench_4min.wav`) unter dem Maximum —
der Boden sitzt damit rund **98 dB** unter der lautesten Zelle, nicht 80, und
die 16 dB Reserve sind aufgebraucht.

**Der Befund steht trotzdem, die Herleitung nicht mehr.** Gemessen wurde
`soxr-s16` gegen `soxr-flt` über 180 FLEURS-Aufnahmen — bitgleiche Ausgabe —,
aber unter dem alten Boden, der den Unterschied per Konstruktion wegklemmte.
Unter dem heutigen Boden ist die Messung nicht wiederholt worden. Als offene
Frage notiert; sie beträfe nur sehr leise ausgesteuertes 16-Bit-Material, und
§3 sagt, was dort ohnehin dominiert.
...
Das Argument überlebt den Perzentil-Boden unverändert, weil auch das Perzentil
relativ zum selben Spektrogramm liegt — ein konstanter Summand verschiebt
Statistik und Zellen gleichermaßen. (Was *nicht* mitskaliert, ist das
int16-Quantisierungsrauschen; siehe die offene Frage oben.)
...
## 6. Der Mel-Pfad und sein Clamp-Boden

Eine Stufe hinter dem Audiopfad, im Merkmalspfad, sitzt der **einzige geprüfte
Eingriff dieses Projekts, der etwas gebracht hat**: der Boden, auf den das
log-Mel geklemmt wird, kam aus dem Maximum der ganzen Eingabe, sodass eine
einzige laute Zelle ihn für den gesamten Durchgang festlegte — ein Türknall
kostet so **+1,52 WER-Punkte** [+0,78, +2,26]. Seit dem 23.08.2026 nimmt die
Engine ein Perzentil, was den Schaden beseitigt und auf sauberem Material nichts
kostet.

Das ist eine eigene Messlage und steht in
[voxtral-mel-clamp-boden.md](voxtral-mel-clamp-boden.md): der Schaden, die
Kosten, die Perzentilwahl, was dagegen spricht, die Grenze bei überwiegend
stillen Pässen samt der dort verworfenen Kappe — und die Vorgeschichte, warum
das log-Mel über die ganze Datei statt je 30-s-Block gerechnet wird.
...
3. **Eine Referenz, die durch Korrektur eines Transkripts entstanden ist, ist an
   diesen Arm angelehnt** — hier mit **0,25 CER-Punkten** beziffert, gemessen,
   indem derselbe Pfad mit anderem Resampler gegen dieselbe Referenz antritt.
   Vergleiche laufen deshalb gegen `p2-soxr`, nicht gegen `p0-ist`;
   Streitstellen listet `adjudicate.py` mit Zeitmarke zum Nachhören. Was das
   für die zweite Handreferenz bedeutet, steht in `andere-asr-engines.md`.
4. **Reproduzierbarkeit ersetzt keine Stichprobe.** Die Messung, die den
   ...
   falsch. Dasselbe beim Clamp-Boden: dass der Knall-Schaden an drei Positionen
   „exakt gleich groß" war, belegte Determinismus, nicht Allgemeinheit.
...
6. **Ein Punktschätzer ohne tragfähiges Intervall zeigt in die falsche
   Richtung.** Der Whisper-Arm des Transienten-Tests wechselte zwischen 8 und
   24 Strömen das Vorzeichen — das Verhalten von Rauschen, nicht von einem
   Effekt. Zahlen in `voxtral-mel-clamp-boden.md`, Abschnitt 7.
...
| `mel_stream.py` · `voxpopuli_*.py` · `mel_outlier_gap.py` · `whisper_spike_streams.py` | die Werkzeuge des Clamp-Bodens; ihre Tabelle steht in [voxtral-mel-clamp-boden.md](voxtral-mel-clamp-boden.md) |
